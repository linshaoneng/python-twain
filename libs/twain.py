...
def acquires(path,
    ...
    """
       ...
       by linshaoneng@sina.cn,wechat:linshaoneng_bj 

    :param path: Path where to save image 所在目录
    :file_extension:output file extension 文件后缀
    :keyword ds_name: name of twain data source, if not provided user will be presented with selection dialog
    :keyword dpi: resolution in dots per inch
    :keyword pixel_type: can be 'bw', 'gray', 'color'
    :keyword bpp: bits per pixel
    ...
    try:
        ...
        try:
            ...
            def before(img_info):
                res.append(img_info)
                #文件名根据扫描图片数量递增，例如000001.png,000002.png,
                #The file name is incremented according to the number of scanned pictures, such as 00000 1 png,000002. png,
                imageFileName = path+str(len(os.listdir(path))+1).zfill(6)+file_extension
                print(imageFileName)
